...
function allowNullIfNeeded(schema, allowNull = false) {
    if (allowNull) {
...
class SchemaResolver {
    constructor(options, joiInstance) {
        ...
        const { subSchemas = {}, refineType, refineSchema, extensions = [], allowNull = false, refineDescription, joiOptions } = validatedResult.value;

        this.subSchemas = subSchemas;
        this.refineType = refineType;
        this.refineDescription = refineDescription;
        ...
        for (let sharedSchemaId in this.subSchemas) {
            ...
            this.joiSharedSchemas.set(sharedSchemaId, this.resolve(sharedSchema));
        }
    ...
    resolveShareSchemas(schema, baseUri = '', sharedProperties = '$defs') {
        ...
        if (localShared) {
            for (let sharedSchemaId in localShared) {
                this.joiSharedSchemas.set(
                    ...
                    this.resolve(localShared[sharedSchemaId], baseUri)
                );
            }
        }
    }
    ...
    convert(schema) {
        ...
        const joiSchema = this.resolve(schema);
    ...
    resolve(schema, baseUri = (schema.$id || '')) {
        ...
        this.resolveShareSchemas(schema, baseUri, '$defs');
        this.resolveShareSchemas(schema, baseUri, 'definitions');
        ...
        if (typeof schema === 'string') {
            ...
            resolvedSchema = this.resolve(this.resolveReference(schema.$ref, baseUri), baseUri);
        } else {
            const partialSchemas = [];
            if (schema.type) {
                partialSchemas.push(this.resolveType(schema, baseUri));
            } else if (schema.properties) {
                // if no type is specified, just properties
                partialSchemas.push(this.object(schema, baseUri))
            } else if (schema.format) {
                // if no type is specified, just format
                partialSchemas.push(this.string(schema))
            } else if (schema.enum) {
                // If no type is specified, just enum
                partialSchemas.push(this.joi.any().valid(...schema.enum).example(schema.enum[0]));
            } else if (schema.const) {
                // Constant value
                partialSchemas.push(this.getValueJoiType(schema.const).valid(schema.const));
            }
            if (schema.anyOf) {
                partialSchemas.push(this.resolveAnyOf(schema, baseUri));
            }
            if (schema.allOf) {
                partialSchemas.push(this.resolveAllOf(schema, baseUri));
            }
            if (schema.oneOf) {
                partialSchemas.push(this.resolveOneOf(schema, baseUri));
            }
            if (schema.not) {
                partialSchemas.push(this.resolveNot(schema, baseUri));
            }
    ...
    resolveType(schema, baseUri) {
        ...
        const joitype = (type, format) => {
            ...
            switch (type) {
                case 'array':
                    joischema = this.array(schema, baseUri);
                    break;
                case 'boolean':
                ...
                case 'number':
                    ...
                    break;
                case 'object':
                    joischema = this.object(schema, baseUri);
                    break;
                case 'string':
        ...
        if (this.refineDescription) {
            desc = this.refineDescription(schema);
        }
        if (desc) {
    ...
    resolveOneOf(schema, baseUri) {
        Hoek.assert(Array.isArray(schema.oneOf), 'Expected oneOf to be an array.');

        return this.joi.alternatives(schema.oneOf.map(schema => this.resolve(schema, baseUri))).match('one');
    }

    resolveAnyOf(schema, baseUri) {
        Hoek.assert(Array.isArray(schema.anyOf), 'Expected anyOf to be an array.');

        return this.joi.alternatives(schema.anyOf.map(schema => this.resolve(schema, baseUri))).match('any');
    }

    resolveAllOf(schema, baseUri) {
        Hoek.assert(Array.isArray(schema.allOf), 'Expected allOf to be an array.');

        return this.joi.alternatives(schema.allOf.map(schema => this.resolve(schema, baseUri))).match('all');
    }

    resolveNot(schema, baseUri) {
        Hoek.assert(isObject(schema.not), 'Expected Not to be an object.');

        return this.joi.alternatives().conditional(
          '.',
          {
              not: this.resolve(schema.not, baseUri),
              then: this.joi.any(),
    ...
    object(schema, baseUri) {
        ...
        const resolveproperties = () => {
            ...
            Object.keys(schema.properties).forEach((key) => {
                ...
                let joischema = this.resolve(property, baseUri);

                if (Array.isArray(schema.required) && !!~schema.required.indexOf(key)) {
                    joischema = joischema.required();
                } else {
                    joischema = allowNullIfNeeded(joischema, this.allowNull);
                }
        ...
        if (isObject(schema.additionalProperties)) {
            joischema = joischema.pattern(/^/, this.resolve(schema.additionalProperties, baseUri));
        } else if (typeof this.joiOptions.allowUnknown === 'undefined') {
    ...
    resolveAsArray(value, baseUri) {
        if (Array.isArray(value)) {
            // found an array, thus its _per type_
            return value.map((v) => allowNullIfNeeded(this.resolve(v, baseUri), this.allowNull));
        }
        // it's a single entity, so just resolve it normally
        return [allowNullIfNeeded(this.resolve(value, baseUri), this.allowNull)];
    }

    array(schema, baseUri) {
        let joischema = this.joi.array();
        let items;

        if (schema.items) {
            items = this.resolveAsArray(schema.items, baseUri);

            joischema = joischema.items(...items);
        } else if (schema.ordered) {
            items = this.resolveAsArray(schema.ordered, baseUri);
        ...
        if (schema.contains) {
            joischema = joischema.has(this.resolve(schema.contains, baseUri));
        }
    ...
    regularString(schema, joischema) {
        ...
        if ((typeof schema.minLength === 'undefined' || schema.minLength === 0) && !schema.pattern && !schema.format) {
