...
function allowNullIfNeeded(schema, { allowNull = false, forbidArrayNull = true }) {
    if (allowNull) {
...
class SchemaResolver {
    ...
    resolve(schema, resolveOptions = {}, baseUri = (schema.$id || '')) {
        ...
        if (typeof schema === 'string') {
            ...
            resolvedSchema = this.resolveType({ type: schema }, resolveOptions, baseUri);
        } else if (schema.$ref) {
            if (schema.$ref.trim() === '#') {
                return baseUri ? this.joi.link(`#${normalizedId(baseUri)}`) : this.joi.link('/');
            }

            resolvedSchema = this.resolve(this.resolveReference(schema.$ref, baseUri), resolveOptions, baseUri);
        } else {
            const partialSchemas = [];
            if (schema.type) {
                partialSchemas.push(this.resolveType(schema, resolveOptions, baseUri));
    ...
    resolveType(schema, resolveOptions, baseUri) {
        let joischema;
        ...
        const joitype = (type, options, format) => {
            ...
            switch (type) {
                case 'array':
                    joischema = this.array(schema, options, baseUri);
                    break;
                case 'boolean':
    ...
    resolveProperties(jsonSchema, resolveOptions, baseUri) {
        ...
        Object.keys(jsonSchema.properties).forEach((key) => {
            ...
            let joischema = this.resolve(property, resolveOptions, baseUri);

            if (Array.isArray(jsonSchema.required) && !!~jsonSchema.required.indexOf(key)) {
                joischema = joischema.required();
                if (resolveOptions.strictRequired) {
                    joischema = joischema.invalid(null, '');
                }
            } else {
                joischema = allowNullIfNeeded(joischema, resolveOptions);
            }
    ...
    resolveAsArray(value, resolveOptions, baseUri) {
        if (Array.isArray(value)) {
            // found an array, thus its _per type_
            return value.map((v) => allowNullIfNeeded(this.resolve(v, resolveOptions, baseUri), resolveOptions));
        }
        // it's a single entity, so just resolve it normally
        return [allowNullIfNeeded(this.resolve(value, resolveOptions, baseUri), resolveOptions)];
    }

    array(schema, resolveOptions, baseUri) {
        let joischema = this.joi.array();
        let items;

        if (schema.items) {
            items = this.resolveAsArray(schema.items, resolveOptions, baseUri);

            joischema = joischema.items(...items);
        } else if (schema.ordered) {
            items = this.resolveAsArray(schema.ordered, resolveOptions, baseUri);
