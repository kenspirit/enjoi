...
class SchemaResolver {
    ...
    resolveProperties(jsonSchema, resolveOptions, baseUri) {
        ...
        Object.keys(jsonSchema.properties).forEach((key) => {
            ...
            if (Array.isArray(jsonSchema.required) && !!~jsonSchema.required.indexOf(key)) {
                ...
                joischema = allowNullIfNeeded(joischema, resolveOptions.allowNull);
            }
    ...
    resolveAsArray(value, resolveOptions, baseUri) {
        if (Array.isArray(value)) {
            ...
            return value.map((v) => allowNullIfNeeded(this.resolve(v, resolveOptions, baseUri), resolveOptions.allowNull));
        }
        // it's a single entity, so just resolve it normally
        return [allowNullIfNeeded(this.resolve(value, resolveOptions, baseUri), resolveOptions.allowNull)];
    }
