...
class SchemaResolver {
    constructor(options, joiInstance) {
        ...
        const { subSchemas = {}, refineType, refineSchema, strictMode, useDefaults, extensions = [], allowNull = false, refineDescription, joiOptions } = validatedResult.value;
    ...
    resolve(schema, baseUri = (schema.$id || '')) {
        ...
        if (this.joiOptions.noDefaults !== true && this.useDefaults && schema.default !== undefined) {
    ...
    resolveType(schema, baseUri) {
        ...
        if (example) {
            joischema = joischema.example(example);
        }
    ...
    array(schema, baseUri) {
        ...
        const resolveAsArray = (value) => {
            if (Array.isArray(value)) {
                ...
                return value.map((v) => this.resolve(v, baseUri));
            }
            // it's a single entity, so just resolve it normally
            return [this.resolve(value, baseUri)];
        }
    ...
    regularString(schema, joischema) {
        ...
        if (schema.minLength === 0 && !schema.pattern && !schema.format) {
